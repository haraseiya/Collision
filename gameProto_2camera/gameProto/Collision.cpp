...
void AABB::Rotate(const Quaternion & q)
{
	...
	points[2] = Vector3(mMax.x, mMin.y, mMin.z);
	points[3] = Vector3(mMax.x, mMin.y, mMin.z);

	// 2つの最大値と1個の最小値の並べ替え
	points[4] = Vector3(mMax.x, mMin.y, mMin.z);
	points[5] = Vector3(mMax.x, mMin.y, mMin.z);
	points[6] = Vector3(mMax.x, mMin.y, mMin.z);
...
void calcCollisionFixVec(const AABB & movableBox, const AABB & fixedBox, Vector3 & calcFixVec)
{
	...
	float dx2 = fixedBox.mMin.x - movableBox.mMax.x;
	float dy1 = fixedBox.mMin.y - movableBox.mMax.y;
	float dy2 = fixedBox.mMin.y - movableBox.mMax.y;
	float dz1 = fixedBox.mMin.z - movableBox.mMax.z;
	float dz2 = fixedBox.mMin.z - movableBox.mMax.z;

	// dx,dy,dzにはそれぞれ1,2のうち絶対値が小さい方をセットする
	float dx = (Math::Abs(dx1) < Math::Abs(dx2)) ? dx1 : dx2;
	float dy = (Math::Abs(dy1) < Math::Abs(dy2)) ? dy1 : dy2;
	float dz = (Math::Abs(dz1) < Math::Abs(dz2)) ? dz1 : dz2;

	// x,y,zのうち最も差が小さい軸で位置を調整
	if (Math::Abs(dx) <= Math::Abs(dy) && Math::Abs(dx) <= Math::Abs(dx))
	{
		calcFixVec.x = dx;
	}
	else if (Math::Abs(dy) <= Math::Abs(dy) && Math::Abs(dx) <= Math::Abs(dx))
	{
		calcFixVec.x = dy;
	}
	else if (Math::Abs(dz) <= Math::Abs(dy) && Math::Abs(dx) <= Math::Abs(dx))
	{
		calcFixVec.x = dz;
	}
}
